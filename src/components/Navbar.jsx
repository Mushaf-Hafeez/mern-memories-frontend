...
const Navbar = () => {
  ...
  return (
    <header className="px-10 md:px-20 lg:px-40 py-5 flex flex-col font-poppins">
          ...
          {token ? (
            <div className="flex items-center gap-2">
              <span
                ...
                className="h-10 w-10 text-center pt-1.5 text-lg bg-primary text-Background rounded-full cursor-pointer"
              >
                {name.slice(0, 2)}
              </span>
              <Link
                to={"/create"}
                className="px-3 py-2 rounded flex items-center gap-2 bg-primary text-Background cursor-pointer active:bg-primary/60"
              >
                Create Post
                <IoAdd size={20} />
              </Link>
      ...
      {isOpen && (
        <nav className="flex items-center justify-center gap-2 md:hidden transition-all ease-in-out duration-200">
          {token ? (
            <div className="flex flex-col items-end gap-2 w-full">
              <Link
                onClick={() => setIsOpen(false)}
                to={"/create"}
                className="px-3 py-2 rounded border-b-1 border-secondary text-primary border-b-primary cursor-pointer"
              >
                Create Post
              </Link>
              <button
